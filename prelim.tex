...
\red{
    These are precisely the conditions required to perform the skein theory on $A$-modules 
    to define the tensor product on $\CC_A$.
    Etale is the same thing as multiplication having a half-inverse, which means we can pop $A$-bigons.
    Connected means theres only one $A$-cap and one $A$-cup.
    This lets us wiggle enough to perfrom the proof that $M\otimes_A N$ is well-defines.
...
\red{Needs to be placed properly.}
\blue{

\begin{proposition}
    Suppsoe $\CC$ has simple unit, $\DD$ is unitary, and $F:\CC\to \DD$ is a $\dagger$-functor.
    Then $\ol{\CC}$ is unitary, and $F$ descends to a $\dagger$-embedding $F:\ol{\CC}\to \DD$ such that
    \[ diagram\]
\end{proposition}

\begin{lemma}\label{lem:exact-functor}
    Let $\FF:\CC\to\DD$ be a monoidal functor with faithful exact right adjoint $R$. 
    If we define $A\coloneqq R(\unit)$, then there is an isomorphism $K$ such that the diagram
    \[
...
\begin{lemma}[Half-braid]\label{eq:general-half-braid}
...
\[    
    g_3 = ..., \quad\text{and}\quad g_4 = ...
\]
From \cite{g2_graphs} we see that at both levels $k=3,4$ we have
\[
...
\begin{remark}\label{rem:Pg-properties}
    ...
    The behavior of the $P_{g_k}$ will be captured by $\skein{/skein_figs/Pg}{0.1}$ in $\DD_k$.
