...
This lets us wiggle enough to perfrom the proof that $M\otimes_A N$ is well-defined.
...
\red{Need to be placed properly.

and led into}


\begin{lemma}
    Suppsoe $\CC$ has simple unit, $\DD$ is unitary, and $\FF:\CC\to \DD$ is a $\dagger$-functor.
...
the braid in $\CC$ is mapped to a braiding of the image of $\CC$ in $\CC_A$.
Since the free functor is in general not full, we cannot extend this to a braidng on all of $\CC_A$,
