...
Subsection~\ref{subsec:extend-level-4} discusses the representation theory which led us to 
...
\]
which takes a simple $\otimes$-generator to a simple $\otimes$-generator.
Combine this with the facts that $\ol{\Rep(U_{q_4}(\gg_2))}_{A_4}$ contains a subcategory equivalent to $\Vecc{\Z_2}$,
and that both of the grouplike simples are subobjects of the square of the $\otimes$-generator.
One concludes that $\ol{\Rep(U_{q_4}(\gg_2))}_{A_4}$ contains a projection onto a $\Z_2$-like simple object 
...
the idempotent whose role is played by $\skein{skein_figs/Pg}{0.08}$.
To this end, we take a similar approach to the one of the previous subsection.
...
\red{We only start with (Recouple) - (decBigon).
All other relations were discovered.}

\red{Give a table of nonzero coordinates like at level 3, in polynomial representation.}


\red{ Lead into this. }
\red{
    We haven't referenced the representation theory going on.
    Either remove refrences to $g$, or phrase alternately.
}
...
\begin{enumerate}
    \item The object $g$ is $\Z_3$-like and simple and therefore has fusion graph given by the 
    oriented edges of the graph in Figure~\ref{fig:new-graphs-lvl3}.
...
\end{enumerate}
The existence and behavior of $T_g$ must be captured by the image of $P_g$ in the GPA, which is all we have access to. 
This is indeed the case, as the only coordinates of the projection in the GPA which are nonzero are at those basis vectors 
\[
    (i \to\blank\to j, i \to\blank\to j)
\]
where $i\to j$ is a directed edge of the $g$-fusion graph. 
For $i=j=1$ there are three possible values for $\blank$; pairing them gives 9 pairs. 
For $i,j\neq 1$ there are four possible values for $\blank$; pairing them gives 16 pairs. 
The columns of Table~\ref{tab:lvl-3-proj-coefs} give numerical approximations to the coordinates of the projection, 
...
\begin{table}
    ...
    \begin{tabular}{|cccccc|} \hline
        $1\to\blank\to1$ & $4\to\blank\to4$    &    $2\to\blank\to6$    &    $3\to\blank\to5$ &  $2\to\blank\to6$   &  $3\to\blank\to5$ \\ \hline\hline
        $2.22$  & $2.22$  & ---   & ---& --- & --- \\
        $-2.22$ & $-2.22$  & ---  & ---& --- & --- \\
        $-2.22$ & $-2.22$  & ---  & ---& --- & --- \\
        $2.22$  & $2.22$  & --- & ---& --- & --- \\
                ...
                &  & --- & ---& --- & --- \\ \hline
    \end{tabular}
    \caption{Level 4 projection embedding coefficients.\\ 
    \red{Change to polynomial representations, then give vars <-> vals correspondence.}}
    \label{tab:lvl-4-proj-coefs}
\end{table}




As an immediate consequence of Theorems~\ref{thm:G2-level3-GPA-emb} and \ref{thm:D4-GPA-emb} we have the following corollary.
...
However we use numerical approximations here, and relegate the actual numbers to the attached Mathematica files.
...
Despite the more difficult numbers, we are able to recover some of the strutcure of the fusion graph in the GPA coordinates.
...
\begin{theorem}\label{thm:D3-GPA-emb}
    There exists an element $P_3  \in\Hom_{\GPA(\Gamma_3)}(2\to 2)$ satisfying the relations (), (), (), (), and (),
    with structure constants given in the attached Mathematica notebook.
\end{theorem}
\begin{proof}
