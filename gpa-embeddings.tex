...
\end{theorem}
\begin{proof}
    We first construct a monoidal functor $\GG_2(q_3) \hookrightarrow \GPA(\Gamma_4)$, and then semisimplify.
...
\]
which takes a simple $\otimes$-generator $X_4$ to a simple $\otimes$-generator $Y_4$.\footnote{
...
One concludes that $\Hom_{\ol{\Rep(U_{q_4}(\gg_2))}_{A_4}}(Y^{\otimes2}\to Y^{\otimes2})$ contains a projection onto a $\Z_2$-like simple object 
...
That is, we assume that for some $b_1,\dots,b_N \in \C$ we have
\[
    F_4\left( \skein{skein_figs/dec_tet_5}{0.08} \right) = \sum_{i=1}^N b_i (p'_i,q'_i).
\]
We may push the relations (Recouple) - (decBigon) through $F_4$, and obtain a set of equations 
in the $b_i$.
...
\red{
Table~\ref{tab:lvl-4-proj-coefs} holds numerical approximations of the nonzero projection coordinates
...
The columns of Table~\ref{tab:lvl-3-proj-coefs} give the values of the coordinates of the projection, 
...
\subsection{New Relations}
When finding GPA embeddings of $\DD_3$ and $\DD_4$, we did not use the relations 
(Swap), (Change of Basis), (decTrigon), (decTetragon), or (decPentagon).
In fact, the structure constants in these relations were unknown before exploring our GPA embedding.
Instead, we used only the relations (Schur 0), (Schur 1), (decStick), and (Half-braid).
This large number of linear equations was enough to pin down the images $F_k\left( \skein{skein_figs/dec_tet_5}{0.08} \right)$.

Of primary importance were the (Half-braid) equations.
These follow from Lemma~\ref{lem:general-half-braid}.
We translate this lemma into the hypothesis that our projection $\skein{skein_figs/dec_tet_5}{0.08}$ satisfies the relation
\begin{equation*}
    \skein{/skein_figs/hb_top}{0.11} = \skein{/skein_figs/hb_bottom}{0.15}
\end{equation*}
where the unoriented colored strand indicates that the equation 
holds whenever we pick the same orientation for the left and right sides.

Now, in order to discover the new relations (Swap), (Change of Basis), (decTrigon), (decTetragon), and (decPentagon),
we hypothesize the form such a relation should take, impose that form, and solve for the structure constants.
For example, we would suppose based on representation theory, that an equation of the form
\[
...
Discovering the (decTrigon) structure constants is similarly reduced to solving a system of likear equations of the form
\[
